...
This change is a render-fidelity follow-up to the archived change
`adapt-factura-pdf1-to-fsframework` (2026-07-21). It locks in the
lesson from the post-archive audit (Engram obs #367): the rendering
spec MUST assert that each setting has a distinctive effect on the
rendered output, not merely on the admin UI. This delta adds 17
feature-level scenarios (rows 1, 2, 3, 4, 5, 6, 7, 8, 11, 12, 13, 14,
15, 16, 17, 18, 19 of the proposal's feature table). The two text-block
features (rows 9 and 10) live in the `invoice-pdf-settings` delta
because they are setting-driven layout, not standalone render
features.

## MODIFIED Requirements

_No existing requirement in the source-of-truth `invoice-pdf-rendering/spec.md`
is being modified. The existing 5 requirements (PDF binary validity,
Twig template resolution, public service contract, dependency
isolation, golden PDF regression) all remain valid as written; the
rendering pipeline they describe is preserved unchanged._
...
_None. The existing 5 requirements of `invoice-pdf-rendering` remain
valid; this change is a pure addition of feature-level scenarios._
