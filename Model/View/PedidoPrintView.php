...
final class PedidoPrintView implements ClientDocumentPrintViewInterface
{
    use PrintViewFormattingTrait;

    /** @var (callable(int): (\FSFramework\model\pedido_cliente|false))|null */
    private static $documentResolver = null;

    /** @var (callable(\FSFramework\model\pedido_cliente): array<string, mixed>)|null */
    ...
    private function __construct(
        private readonly \FSFramework\model\pedido_cliente $pedido,
    ...
    {
        $pedido = self::resolveDocument($id);
        if ($pedido === false) {
    ...
    /** @param callable(int): (\FSFramework\model\pedido_cliente|false)|null $documentResolver */
    ...
    {
        return (int) $this->pedido->idpedido;
    }
    ...
    {
        return self::formatMoney($this->divisa, $this->locale, (float) $this->pedido->total);
    }

    public function getSubtotalFormatted(): string
    {
        return self::formatMoney($this->divisa, $this->locale, (float) $this->pedido->neto);
    }
    ...
    /** @return \FSFramework\model\pedido_cliente|false */
    private static function resolveDocument(int $id): \FSFramework\model\pedido_cliente|false
    {
        ...
        return (new \FSFramework\model\pedido_cliente())->get((string) $id);
    }

    private static function build(\FSFramework\model\pedido_cliente $pedido): self
    {
        if (self::$relatedResolver !== null) {
            $related = (self::$relatedResolver)($pedido);
    ...
    {
        if (!class_exists(\FSFramework\model\pedido_cliente::class, false)) {
